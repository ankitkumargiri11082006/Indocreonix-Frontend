...
function CareerDashboardPage() {
  ...
  const filteredOpenings = useMemo(() => {
    ...
    return openings.filter((opening) => String(opening.type || '').toLowerCase() === openingFilter)
  ...
  useEffect(() => {
    ...
    async function loadApplications() {
      ...
      try {
        ...
        setOpenings(Array.isArray(opportunitiesResult.items) ? opportunitiesResult.items : [])
      } catch (err) {
        if (!mounted) return
  ...
  return (
    <section className="portal-dashboard-shell">
      <SEO title="Career Dashboard" description="Track your applications and progress." noindex={true} />
      <div className="portal-dashboard-topbar">
        <div>
          <p className="portal-kicker">Career Dashboard</p>
          <h1>Welcome, {user.name || 'Candidate'}</h1>
          <p>Track every role you applied for and monitor hiring progress live.</p>
        </div>
        <div className="portal-dashboard-actions">
          ...
          <Link className="btn btn-secondary" to="/project/dashboard">Project Dashboard</Link>
          <button
            type="button"
      ...
      <div className="portal-metric-grid" aria-label="Career dashboard highlights">
        ...
        <article className="portal-metric-card">
          ...
          <span>{openingFilter === 'all' ? 'All opportunities shown' : `Filtered by ${openingFilter}`}</span>
        </article>
      </div>
      ...
      <div className="portal-dashboard-grid">
        {applications.length ? (
          applications.map((item) => (
            <article key={item.id || `${item.role}-${item.submittedAt}`} className="portal-status-card">
          ...
          <article className="portal-status-card">
            ...
            <p className="portal-status-note">Apply to open roles and track progress from this dashboard.</p>
      ...
      <div className="portal-project-section">
        <div className="portal-section-head">
          ...
          <div className="portal-filter-row" role="tablist" aria-label="Opening filters">
            {[
              { value: 'all', label: 'All' },
              ...
              <button
                ...
                className={openingFilter === filterOption.value ? 'portal-filter-chip portal-filter-chip-active' : 'portal-filter-chip'}
        ...
        <div className="portal-dashboard-grid">
          {filteredOpenings.length ? (
            filteredOpenings.map((opening) => (
              <article key={opening._id} className="portal-status-card portal-opening-card">
                <div className="portal-status-head">
                ...
                <p className="portal-status-note">Location: {opening.location || 'Not specified'}</p>
                <Link
            ...
            <article className="portal-status-card">
              <p className="portal-status-note">No active openings found for this filter right now.</p>
            </article>
          )}
        </div>
      </div>
