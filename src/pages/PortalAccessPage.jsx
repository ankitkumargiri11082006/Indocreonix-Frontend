...
function PortalAccessPage({ embedded = false, nextPath = '', onAuthenticated }) {
  ...
  useEffect(() => {
    ...
    const shouldOpenProfile = String(searchParams.get('profile') || '').toLowerCase() === 'edit'
  ...
  function goToPostAuthDestination(user) {
    ...
    const safeNextFromProp = safeNextPath.startsWith('/') && !safeNextPath.startsWith('//')
    if (safeNextFromProp) {
    ...
    const isSafeNextPath = requestedNext.startsWith('/') && !requestedNext.startsWith('//')

    if (isSafeNextPath) {
      if (requestedNext.startsWith('/careers') && !user?.access?.career) {
        navigate(user?.defaultDashboard === 'project' ? '/project/dashboard' : '/career/dashboard')
        return
      }
  ...
  async function onAvatarFileChange(event) {
    ...
    try {
      ...
      setError(err.message || 'Could not process image. Please try another photo.')
    } finally {
  ...
  return (
    <>
      {!embedded ? <SEO title="Portal Access" description="Single page access for portal sign in and sign up." noindex={true} /> : null}
      <section className={embedded ? 'portal-shell portal-shell-login portal-shell-embedded' : 'portal-shell portal-shell-login'}>
        <div className="portal-grid">
          <aside className="portal-hero-card">
            ...
            <p>
              Continue with Google in seconds, or verify with OTP to create your email-password access with enterprise-grade session control.
            </p>
          ...
          <article className="portal-auth-card">
            {currentUser ? (
              <div className="portal-session-card">
                <div className="portal-session-head">
                  {currentUser.avatarUrl ? (
                    <img src={currentUser.avatarUrl} alt={currentUser.name || 'Profile'} className="portal-session-avatar" />
                  ) : (
                    <span className="portal-session-avatar portal-session-avatar-fallback" aria-hidden="true">
                      {getUserInitials(currentUser)}
                    </span>
                  )}
                  <div>
                    <h2>Profile Session Active</h2>
                    <p className="portal-auth-subtitle">You are already logged in. Login/sign-up forms are hidden until your session expires.</p>
                  </div>
                </div>
                <p className="portal-session-user"><strong>Name:</strong> {currentUser.name || 'Portal User'}</p>
                <p className="portal-session-user"><strong>Email:</strong> {currentUser.email}</p>
                <p className="portal-session-user">
                  <strong>Session Remaining:</strong> {Math.floor(sessionRemainingMs / 60000)}m {Math.floor((sessionRemainingMs % 60000) / 1000)}s
                </p>
                <div className="portal-session-tags" aria-hidden="true">
                  <span>{currentUser?.access?.career ? 'Career Access' : 'Career Locked'}</span>
                  <span>{currentUser?.access?.project ? 'Project Access' : 'Project Locked'}</span>
                </div>
              ...
              <>
            <div className="portal-mode-switch" role="tablist" aria-label="Portal auth modes">
              <button
                type="button"
                className={mode === 'signin' ? 'portal-mode-btn active' : 'portal-mode-btn'}
                onClick={() => {
                  setMode('signin')
                  setError('')
                  setMessage('')
              ...
              <button
                type="button"
                className={mode === 'signup' ? 'portal-mode-btn active' : 'portal-mode-btn'}
                onClick={() => {
                  setMode('signup')
                  setError('')
                  setMessage('')
            ...
            <div className="portal-google-wrap">
              {googleClientId ? (
                <>
                  ...
                  {!googleReady ? <p className="portal-inline-note">Loading Google access...</p> : null}
                  {googleLoading ? <p className="portal-inline-note">Processing Google account...</p> : null}
                </>
              ) : (
                <p className="portal-inline-note portal-inline-warning">
                  Google access is disabled. Add VITE_GOOGLE_CLIENT_ID in frontend env.
                </p>
              )}
            </div>

            <div className="portal-auth-divider"><span>or continue with email flow</span></div>
              ...
              <form className="portal-form" onSubmit={handleSendOtp}>
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) => setSignupForm((prev) => ({ ...prev, track: event.target.value }))}
                  >
        ...
        {isProfileEditorOpen ? (
          <div className="portal-profile-modal-backdrop" role="dialog" aria-modal="true" aria-label="Edit profile">
            <section className="portal-profile-modal">
              <div className="portal-profile-modal-head">
                ...
                <button type="button" className="portal-profile-close" onClick={() => setIsProfileEditorOpen(false)} aria-label="Close profile editor">
                  &times;
                </button>
              </div>
              ...
              <form className="portal-form" onSubmit={onSaveProfile}>
                <div className="portal-profile-avatar-row">
                  {profileDraft.avatarUrl ? (
                    <img src={profileDraft.avatarUrl} alt={profileDraft.name || 'Profile preview'} className="portal-profile-modal-avatar" />
                  ) : (
                    <span className="portal-profile-modal-avatar portal-session-avatar-fallback" aria-hidden="true">
                      {getUserInitials(profileDraft)}
                    </span>
                  )}
                  <div className="portal-action-row">
                    <button type="button" className="btn btn-secondary" onClick={onAvatarBrowseClick}>
                      {profileDraft.avatarUrl ? 'Replace Photo' : 'Add Photo'}
                    </button>
                    {profileDraft.avatarUrl ? (
                      <button type="button" className="btn btn-secondary" onClick={() => setProfileDraft((prev) => ({ ...prev, avatarUrl: '' }))}>
                        Remove
                      </button>
                    ) : null}
                    <input ref={profileFileInputRef} type="file" accept="image/*" className="portal-profile-file-input" onChange={onAvatarFileChange} />
                  </div>
                </div>
                ...
                <div className="portal-profile-grid">
                  ...
                  <label>
                    Email (locked)
                    <input name="email" value={profileDraft.email} disabled readOnly />
                  </label>
                  <label>
                    Phone
                    <input name="phone" value={profileDraft.phone} onChange={onProfileFieldChange} placeholder="+91 xxxxxxxxxx" />
                  </label>
                  <label>
                    Organization
                    <input name="organization" value={profileDraft.organization} onChange={onProfileFieldChange} placeholder="Company / College" />
                  </label>
                  <label>
                    Role
                  ...
                  <label className="portal-profile-grid-full">
                    Bio
                    <textarea name="bio" value={profileDraft.bio} onChange={onProfileFieldChange} rows={3} placeholder="Share your focus and goals" />
                  </label>
                </div>
                ...
                <div className="portal-action-row">
                  <button type="button" className="btn btn-secondary" onClick={() => setIsProfileEditorOpen(false)}>
                    Cancel
                  </button>
